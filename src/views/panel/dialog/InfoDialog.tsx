...
const InfoDialog = ({ children, title, content, close }) => {
    ...
    const closeStyle = getButtonStyle(close?.color);
    const close_onclick = () => {
        closeDialog();
        close?.onclick();
    };
                    ...
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <div
                            ...
                            <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                <button onClick={close_onclick} type="button"
                                        className={`mt-3 inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold ${closeStyle} shadow-sm hover:bg-gray-50 sm:mt-0 sm:w-auto`}>
                                    {close?.text ?? 'No text'}
                                </button>
                            </div>
                        </div>
