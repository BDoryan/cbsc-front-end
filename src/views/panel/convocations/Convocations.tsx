...
const Convocations: React.FC<RouteComponentProps<{}>> = () => {
    ...
    const loadConvocations = async (clear?: boolean) => {
        ...
        try {
            const ROUTE = `${API}convocations`;
            const response = await axios.get(`${ROUTE}` + ((searchTerm && searchTerm.length > 0) ? "/search" : ''), {
                params: {
                    search: searchTerm ?? '',
        ...
        setDialog(
            <ConfirmDialog confirm={{
                ...
                onclick: async () => {
                    ...
                    try {
                        const ROUTE = `${API}convocations`;
                        const response = await axios.delete(`${ROUTE}/${convocation.id}`, {
                            headers: {
                                'Authorization': 'Bearer ' + token_session
                            }
                        });
    ...
    return (
        <>
            <h1 className="text-2xl mb-2 font-bold">
                {imManager() ? 'Gestion des convocations' : 'Mes convocations'}
            </h1>
            <p className="text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {convocations.length > 0 && convocations.map((convocation: any, index: number) => {
                    return (
                        <div key={"convocation" + index}
                            ...
                            <div className="flex flex-col p-4 pb-0">
                                ...
                                <div className="py-6 mb-auto">
                                    <p className="text-sm text-gray-600">
                                        <span
                                            className="text-indigo-600">{convocation.invitations?.length ?? '0'} membre(s)</span> ont
                                        été convié(s) à cet événement.<br/>
                                ...
                                <div className="flex justify-between flex-wrap gap-2 mb-5">
                                    <Link to={"/convocations/" + convocation.id}
                                        ...
                                        <div className="flex gap-2">
                                            {
                                                imManager() && (
                                                    <>
                                                        ...
                                                        <Link to={"/convocations/edit/" + convocation.id}
                                                              className="py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-indigo-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
                                                            <i className="fa-solid fa-pen pe-2 text-sm"></i>
                                                            Modifier
                                                        </Link>
                                                        <button onClick={() => deleteConvocation(convocation)}
                                                                className="py-2 px-4 text-sm font-medium text-red-600 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-red-50 hover:text-red-700 focus:z-10 focus:ring-4 focus:ring-red-100">
            ...
            <div>
                {(!loading && convocations?.length === 0) && (
                    <div className="w-full bg-white border border-gray-200 rounded-lg shadow p-4 text-center">
                        <p className="text-gray-500 text-sm">Aucune convocation trouvée</p>
                    </div>
                )}
            </div>
            {hasMore && (
